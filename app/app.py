...
Original file is located at
    https://colab.research.google.com/drive/1JhEn3PJ6UdxTJg12H5LhJl0WXpIf8_eF
"""
...
zonalstats_csv = DATA_DIR / "zonalstats_distritos.csv"
...
@st.cache_data
def load_data():
    zonal_df = pd.read_csv(zonalstats_csv)
    top_cold = pd.read_csv(ranking_cold_csv)
    top_hot = pd.read_csv(ranking_hot_csv)
    return zonal_df, top_cold, top_hot

zonal_df, top_cold, top_hot = load_data()
...
if page == "📑 Data description":
    ...
    **Contexto:**
    ...
    st.write(f"Archivo de Zonal Stats: `{zonalstats_csv.name}`")
    st.download_button(
        "📥 Descargar tabla completa CSV",
        data=zonal_df.to_csv(index=False),
        file_name="zonalstats_distritos.csv",
        mime="text/csv"
    )
...
# -----------------------------------------------------
    ...
    departamento = st.selectbox(
        ...
        options=["Todos"] + sorted(zonal_df['DEPARTAMENTO'].unique())
        if 'DEPARTAMENTO' in zonal_df.columns else ["Todos"]
    )
    df_filtered = zonal_df.copy()
    if departamento != "Todos" and 'DEPARTAMENTO' in zonal_df.columns:
        df_filtered = df_filtered[zonal_df['DEPARTAMENTO'] == departamento]
